...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('🔔 Uᴘᴅᴀᴛᴇ Cʜᴀɴɴᴇʟ 🔔', url=UPDATE_CHNL_LNK)
                  ]]
        ...
        try:
            ...
            if not dreamxbotz_joined:
                try:
                    ...
                    return
                btn.append([InlineKeyboardButton("⛔️ Jᴏɪɴ Gʀᴏᴜᴘ ⛔️", url=invite_link_default.invite_link)])
            if btn:
                if len(message.command) > 1 and "_" in message.command[1]:
                    kk, file_id = message.command[1].split("_", 1)
                    btn.append([
                        InlineKeyboardButton("♻️ Tʀʏ Aɢᴀɪɴ ♻️", callback_data=f"checksub#{kk}#{file_id}")
                    ])
                    reply_markup = InlineKeyboardMarkup(btn)
                    caption = (
                        f"👋 Hey {message.from_user.mention},\n\n"
                        "<b>🚫 Aᴄᴄᴇꜱꜱ Dᴇɴɪᴇᴅ!</b>\n\n"
                        "Yᴏᴜ ʜᴀᴠᴇɴ’ᴛ ᴊᴏɪɴᴇᴅ ᴀʟʟ ᴏꜰ ᴏᴜʀ <b>🔔 Uᴘᴅᴀᴛᴇ Cʜᴀɴɴᴇʟꜱ</b> ʏᴇᴛ.\n"
                        "ᴛᴏ ᴘʀᴏᴄᴇᴇᴅ, ᴘʟᴇᴀꜱᴇ ᴛᴀᴘ ᴛʜᴇ <b>👇 Jᴏɪɴ Cʜᴀɴɴᴇʟꜱ</b> ʙᴜᴛᴛᴏɴꜱ ʙᴇʟᴏᴡ ᴀɴᴅ ᴍᴀᴋᴇ ꜱᴜʀᴇ ʏᴏᴜ’ʀᴇ ɪɴ ᴀʟʟ ᴏꜰ ᴛʜᴇᴍ.\n\n"
                        "✅ ᴏɴᴄᴇ ᴅᴏɴᴇ, ʜɪᴛ <b>♻️ Tʀʏ Aɢᴀɪɴ</b> ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ."
                    )
    ...
    if not await db.has_premium_access(user_id):
        try:
            ...
            if settings.get("is_verify", IS_VERIFY) and (not user_verified or is_second_shortener or is_third_shortener):
                ...
                buttons = [[
                    InlineKeyboardButton(text="♻️ Cʟɪᴄᴋ Hᴇʀᴇ Tᴏ Vᴇʀɪꜰʏ ♻️", url=verify)
                ],[
        ...
        for file in files:
            ...
            if STREAM_MODE:
                btn = [
                    ...
                    [InlineKeyboardButton('➕ Jᴏɪɴ Uᴘᴅᴀᴛᴇꜱ Cʜᴀɴɴᴇʟ ➕', url=UPDATE_CHNL_LNK)]  # Keep this line unchanged  
                ]
            else:
                btn = [[InlineKeyboardButton('➕ Jᴏɪɴ Uᴘᴅᴀᴛᴇꜱ Cʜᴀɴɴᴇʟ ➕', url=UPDATE_CHNL_LNK)]]
            ...
            filesarr.append(msg)
        k = await client.send_message(chat_id=message.from_user.id, text=f"⚠️ ❌👉 Tʜɪꜱ ꜰɪʟᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ❗ Dᴇʟᴇᴛᴇꜱ ᴀꜰᴛᴇʀ <code>{get_time(DELETE_TIME)}</code> ❗ ꜱᴏ ᴘʟᴇᴀꜱᴇ ꜰᴏʀᴡᴀʀᴅ ɪɴ ᴀɴᴏᴛʜᴇʀ ᴄʜᴀᴛ 👈❌")
    ...
    if not files_:
        ...
        try:
            if STREAM_MODE:
                ...
                btn = [[InlineKeyboardButton('➕ Jᴏɪɴ Uᴘᴅᴀᴛᴇꜱ Cʜᴀɴɴᴇʟ ➕', url=UPDATE_CHNL_LNK)]]  # Keep this line unchanged  
            msg = await client.send_cached_media(
                chat_id=message.from_user.id,
                file_id=file_id,
                protect_content=settings.get('file_secure', PROTECT_CONTENT),
            ...
            k = await msg.reply(
                f"⚠️ ❌👉 Tʜɪꜱ ꜰɪʟᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ❗ Dᴇʟᴇᴛᴇꜱ ᴀꜰᴛᴇʀ <code>{get_time(DELETE_TIME)}</code> ❗ ꜱᴏ ᴘʟᴇᴀꜱᴇ ꜰᴏʀᴡᴀʀᴅ ɪɴ ᴀɴᴏᴛʜᴇʀ ᴄʜᴀᴛ 👈❌",
                quote=True
            )
            await asyncio.sleep(DELETE_TIME)
            await msg.delete()
...
async def trendlist(client, message):
    ...
    additional_message = (
        "⚡️ ᴀʟʟ ᴛʜᴇ ʀᴇꜱᴜʟᴛꜱ ᴀʙᴏᴠᴇ ᴄᴏᴍᴇ ꜰʀᴏᴍ ᴡʜᴀᴛ ᴜꜱᴇʀꜱ ʜᴀᴠᴇ ꜱᴇᴀʀᴄʜᴇᴅ ꜰᴏʀ. "
        "ᴛʜᴇʏ'ʀᴇ ꜱʜᴏᴡɴ ᴛᴏ ʏᴏᴜ ᴇxᴀᴄᴛʟʏ ᴀꜱ ᴛʜᴇʏ ᴡᴇʀᴇ ꜱᴇᴀʀᴄʜᴇᴅ, "
        "ᴡɪᴛʜᴏᴜᴛ ᴀɴʏ ᴄʜᴀɴɢᴇꜱ ʙʏ ᴛʜᴇ ᴏᴡɴᴇʀ."
    )
    formatted_list += f"\n\n{additional_message}"
